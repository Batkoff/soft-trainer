...
CHECK_PASSWORD = '''PGPASSWORD="$POSTGRES_PASSWORD" psql -X -w -h 127.0.0.1 -U trainer -d trainer -Atqc 'SELECT 1' '''
...
def ensure_database():
    ...
    print("[2/4] Запускаю PostgreSQL и проверяю подключение…", flush=True)
    compose("up", "-d", "--wait", "--wait-timeout", "120", "db", check=True)
    probe = compose("exec", "-T", "db", "sh", "-c", CHECK_PASSWORD,
                    capture_output=True, text=True, timeout=30)
    if probe.returncode == 0:
        return
    if "password authentication failed" not in probe.stderr:
        raise RuntimeError("PostgreSQL не прошёл проверку подключения. "
                           "Данные сохранены. Проверьте: docker compose logs --tail=80 db")
    ...
    verify = compose("exec", "-T", "db", "sh", "-c", CHECK_PASSWORD,
                     capture_output=True, timeout=30)
...
def main():
    ...
    try:
        ...
        ensure_database()
        print("[3/4] Собираю приложение…", flush=True)
